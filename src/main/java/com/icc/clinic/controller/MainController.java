...
import com.icc.clinic.controller.UserManagementController;
...
public class MainController {
    ...
    // Method to set current user (called from LoginController)
    public void setCurrentUser(User user) {
        this.currentUser = user;
        
        // Use Platform.runLater to ensure FXML is fully loaded before updating UI
        javafx.application.Platform.runLater(() -> {
            if (tabPane != null && currentUser != null) {
                updateUIForUserRole();
            }
        });
    }
    ...
    private void updateUIForUserRole() {
        ...
        boolean isIT = rbacService.userHasRole(currentUser, "IT");
        boolean isNurse = rbacService.userHasRole(currentUser, "NURSE");
        
        System.out.println("=== ROLE CHECK ===");
        System.out.println("Current user: " + currentUser.getUsername());
        System.out.println("User role: " + (currentUser.getRole() != null ? currentUser.getRole().getName() : "null"));
        System.out.println("Is IT: " + isIT);
        System.out.println("Is Nurse: " + isNurse);
        System.out.println("TabPane is null: " + (tabPane == null));
        
        // Show/hide tabs based on role
        if (tabPane != null) {
            System.out.println("Number of tabs before: " + tabPane.getTabs().size());
            for (Tab tab : tabPane.getTabs()) {
                System.out.println("Tab: " + tab.getText());
            }
            
            // User Management tab - only IT can access
            Tab userManagementTab = tabPane.getTabs().stream()
                .filter(tab -> "User Management".equals(tab.getText()))
                .findFirst().orElse(null);
            
            System.out.println("Found User Management tab: " + (userManagementTab != null));
            
            if (isIT) {
                // Keep only User Management for IT: remove all other tabs
                if (userManagementTab != null) {
                    tabPane.getTabs().setAll(userManagementTab);
                    tabPane.getSelectionModel().select(userManagementTab);
                    
                    // Set current user on UserManagementController
                    setCurrentUserOnUserManagementController();
                }
            } else {
                // Non-IT users cannot see User Management tab
                if (userManagementTab != null) {
    ...
    private void setCurrentUserOnUserManagementController() {
        try {
            System.out.println("MainController: Attempting to set current user on UserManagementController");
            System.out.println("MainController: Current user is: " + (currentUser != null ? currentUser.getUsername() : "NULL"));
            
            if (userManagementController != null) {
                System.out.println("MainController: Found UserManagementController reference, setting current user");
                userManagementController.setCurrentUser(currentUser);
                userManagementController.onTabSelected();
            } else {
                System.out.println("MainController: UserManagementController reference is null, trying to find it");
                // Fallback: try to find it the old way
                Tab userManagementTab = tabPane.getTabs().stream()
                    .filter(tab -> "User Management".equals(tab.getText()))
                    .findFirst().orElse(null);
                
                if (userManagementTab != null && userManagementTab.getContent() != null) {
                    VBox content = (VBox) userManagementTab.getContent();
                    UserManagementController foundController = findUserManagementController(content);
                    if (foundController != null) {
                        this.userManagementController = foundController;
                        foundController.setCurrentUser(currentUser);
                        foundController.onTabSelected();
                        System.out.println("MainController: Found and cached UserManagementController");
                    }
                }
            }
        } catch (Exception e) {
            System.err.println("Error setting current user on UserManagementController: " + e.getMessage());
            e.printStackTrace();
        }
    }
    
    private UserManagementController findUserManagementController(javafx.scene.Node node) {
        // Check if this node has a controller property
        if (node.getProperties().containsKey("controller")) {
            ...
            if (controller instanceof UserManagementController) {
                return (UserManagementController) controller;
            }
        }
        ...
        if (node instanceof javafx.scene.Parent) {
            for (javafx.scene.Node child : ((javafx.scene.Parent) node).getChildrenUnmodifiable()) {
                UserManagementController controller = findUserManagementController(child);
                if (controller != null) {
                    return controller;
                }
            }
    ...
    private void updateActionButtons() {
        ...
        boolean canDeletePatients = rbacService.userHasPermission(currentUser, "PATIENT_DELETE");
        boolean canDeleteAppointments = rbacService.userHasPermission(currentUser, "APPOINTMENT_DELETE");
        
        // Update patient action buttons visibility
        if (actionsColumn != null) {
            // This will be handled in the cell factory
        }
        
        // Update appointment action buttons visibility
        if (appointmentActionsColumn != null) {
            // This will be handled in the cell factory
        }
    }
    ...
    public void initialize() {
        try {
            ...
            if (tabPane != null && patientsListTab != null) {
                tabPane.getSelectionModel().selectedItemProperty().addListener((obs, oldTab, newTab) -> {
                    if (newTab == patientsListTab) {
                        loadPatients();
                    } else if (newTab != null && "User Management".equals(newTab.getText()) && currentUser != null) {
                        // Use Platform.runLater to ensure the tab content is fully loaded
                        javafx.application.Platform.runLater(() -> {
                            setCurrentUserOnUserManagementController();
                        });
                    }
            ...
            // UI will be updated when setCurrentUser() is called via Platform.runLater

            // Make all date pickers non-editable (calendar-only)
            if (dateOfBirthPicker != null) dateOfBirthPicker.setEditable(false);
            if (appointmentDatePicker != null) appointmentDatePicker.setEditable(false);
            if (filterStartDate != null) filterStartDate.setEditable(false);
            if (filterEndDate != null) filterEndDate.setEditable(false);
            if (reportStartDate != null) reportStartDate.setEditable(false);
            if (reportEndDate != null) reportEndDate.setEditable(false);
            if (archivedAppointmentDateFilter != null) archivedAppointmentDateFilter.setEditable(false);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void handleAddPatient() {
        ...
        patient.setDateOfBirth(dateOfBirthPicker.getValue() != null ? dateOfBirthPicker.getValue().toString() : null);
        ...
        ValidationResult result = validationService.validatePatient(patient, false);
    ...
    private String getCellString(Row row, int col) {
        ...
        if (cell.getCellType() == org.apache.poi.ss.usermodel.CellType.NUMERIC) {
            ...
            // For blank or other cell types, return empty string
            return "";
        }
    }
    ...
    private void loadPatients() {
        try {
            System.out.println("[MainController] Starting to load patients...");
            List<Patient> patients = patientService.getAllPatients();
            System.out.println("[MainController] Loaded patients: " + patients.size());
            if (!patients.isEmpty()) {
                Patient first = patients.get(0);
                System.out.println("[MainController] First patient - ID: " + first.getId() + 
                                 ", FirstName: " + first.getFirstName() + 
                                 ", LastName: " + first.getLastName());
            }
            patientsTable.getItems().setAll(patients);
            System.out.println("[MainController] Patients loaded into table successfully");
        } catch (Exception e) {
            System.err.println("[MainController] Error loading patients: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private void handleRefreshAppointments() {
        // Clear date filters and reload all appointments
